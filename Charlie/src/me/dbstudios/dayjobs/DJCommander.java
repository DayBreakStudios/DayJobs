						...
						if (args[1].equalsIgnoreCase("reload") && common.hasPerm(((Player)sender).getDisplayName(), "admin.reload", true)) {
							...
							if (tickets.get(0) == null) {
								...
								for (String subject : tickets) {
									...
									sender.sendMessage(ChatColor.AQUA + "--> (" + ticket[1] + ") '" + subject + "' to '" + ticket[0] + "'");
								}
							}
							...
							if (!common.ifDebug("Player '" + ((Player)sender).getDisplayName() + "' toggled debug mode.")) {
								state = "HIGH";
							} else {
								state = "LOW";
							}
						...
						if (args[1].equalsIgnoreCase("exempt")) {
							...
							if (player != null) {
								if (common.toggleExempt(player)) {
									...
									sender.sendMessage(common.prefix + ChatColor.AQUA + "Toggled exempt on '" + player + "' (" + state + ").");
									common.getPlayer(player).sendMessage(common.prefix + ChatColor.AQUA + ((Player)sender).getDisplayName() +
											" has toggled exempt status on you (" + state + ").");
								} else {
									sender.sendMessage(common.prefix + ChatColor.RED + "Error: " + ChatColor.DARK_AQUA +
				...
				if (args[0].equalsIgnoreCase("admin")) {
					if (args[1].equalsIgnoreCase("change") && common.hasPerm(((Player)sender).getDisplayName(), "admin.change", true)) {
						...
						if (args[2].equalsIgnoreCase("set")) {
							...
							if (type.equalsIgnoreCase("death") && common.hasPerm(((Player)sender).getDisplayName(), "admin.setspawn.deathspawn", true)) {
							...
							} else if (type.equalsIgnoreCase("new") && common.hasPerm(((Player)sender).getDisplayName(), "admin.setspawn.newspawn", true)) {
							...
							if (type.equalsIgnoreCase("death") && common.hasPerm(((Player)sender).getDisplayName(), "admin.tp.deathspawn", true)) {
								sender.sendMessage(common.prefix + ChatColor.AQUA + "Teleporting to respawn location.");
								((Player)sender).teleport(common.getDeathSpawn(((Player)sender).getLocation().getWorld().getName()));
							} else if (type.equalsIgnoreCase("new") && common.hasPerm(((Player)sender).getDisplayName(), "admin.tp.newspawn", true)) {
								sender.sendMessage(common.prefix + ChatColor.AQUA + "Teleporting to new spawn location.");
								((Player)sender).teleport(common.getNewPlayerSpawn(((Player)sender).getLocation().getWorld().getName()));
							} else {
								sender.sendMessage(common.prefix + ChatColor.RED + "Error: " + ChatColor.DARK_AQUA + 
										"Can only teleport to 'new' or 'death' spawn locations.");
							}
